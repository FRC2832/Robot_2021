...
public class AutoNav1 extends CommandGroup {
    ...
    public void execute(){
        ...
        addSequential(new AutoRun(rpm), Math.sqrt(horizontalDistance*horizontalDistance+verticalDistance*verticalDistance-4 *(radius1*radius1))/speed);
        addSequential(new AutoCircle(1,r), calculateTimeBasedOnArcAngle(15.0/8*Math.PI-alpha+theta,radius2));
        addSequential(new AutoRun(rpm), (Math.sqrt(2)*horizontalDistance-24)/speed);
